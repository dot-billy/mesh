...
- Managed per-network Slack Nebula relays with a guided dashboard selector for 1–8 pending or active nodes, optimistic revision checks, exact active-relay addresses, and signed rollout. Selected nodes receive server-only relay configuration; every other node advertises only active selections. Revoking a selected relay removes it automatically. Relay support is experimental upstream, and each relay still needs direct underlay reachability from both peers.
...
3. Save the one-time lighthouse credential and run the displayed enrollment command on that machine. **I saved this — add first member** continues directly to the locked member step for the same network.
4. Save the member credential and run its command. **I saved this — check readiness** opens the network's live deployment checks, which refresh automatically every 10 seconds while visible so activation can converge in place; if redundancy is insufficient, that dialog offers **Add second lighthouse**. Every disclosure close clears its token and install/enroll/activate commands from the page. If an enrollment was created by mistake or the machine will not be used, choose **Cancel enrollment** while it is still pending instead of revoking an identity that never existed.
...
9. Open **Network relays** when direct peer tunnels are unreliable. Select 1–8 managed machines with underlay reachability from both sides of the difficult path. Pending selections begin relaying only after enrollment; Mesh configures Nebula but does not prove public-edge reachability or relay capacity.
...
`mesh-release` generates separate offline Ed25519 root and release-role keys, creates canonical versioned roots, assembles dual-threshold root updates, and signs the exact bytes of strict channel or release manifests. `meshctl verify-release` remains a local release-signature diagnostic; `mesh-install` additionally starts from its separately authenticated compiled root and durably replays root rotation and revocation before accepting release metadata. Changing even manifest whitespace invalidates its signatures.
...
`MESH_DARWIN_NATIVE_BUNDLE`, it additionally
