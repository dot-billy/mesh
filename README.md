...
- A two-stage Darwin `amd64`/`arm64` release boundary: deterministic unsigned bundle-v1 USTARs bind a production-identity `meshctl`, reproducible security-patched thin Nebula Mach-O executables from the authenticated source/patch/toolchain lock and layered Darwin output lock, plus reviewed launchd assets. Protected native verification emits a receipt for three exact Developer ID candidates; Linux assembly proves signature-region-only replacement and emits deterministic signed bundle v2. Production-policy inspection reconstructs every final archive byte; a Syft/SPDX/Grype/Gitleaks gate emits one strict package receipt, and release authoring fails closed without both that receipt and the fresh native code-signing receipt. No approved identity or real signed artifact exists, and this does not claim native installation, launchd activation, extended-ACL enforcement, notarization, runtime telemetry, or installed-host support.
...
each final signed Darwin bundle-v2 candidate, then review the
...
gate](docs/windows-package-security.md), [Darwin final signed-bundle security
...
installer. The Darwin final gate does the same for exact signed Mach-O and
launchd assets; release authoring additionally requires the native receipt for
the three exact executables under the compiled policy. It does not claim
native installation, activation, protected signing execution, or
...
The cutover is stopped-server, one-way, and archive-driven: create and catalog a fresh current control-v14 JSON backup, run `mesh-storage migrate` with the migration-role DSN, complete the required cluster-admin function-ownership transfer and `mesh_migrate` grant audit, run `import-backup` and `verify` with the import-role DSN and exact catalog-selected backup ID, then start one canary using the runtime-role DSN. The importer also accepts authenticated bound control-v2 through control-v14 archives and preserves older documents exactly so the first upgraded server startup can perform any remaining ordered topology, managed-DNS, managed-relay, CA-rotation, firewall-rollout, firewall-pause, route-transfer, route-profile, route-policy, native-DNS, firewall-scope, and security-group migrations. A fresh import initializes the separate empty telemetry document automatically. An already-imported database upgraded to migration 002 must run `initialize-runtime-telemetry` once with the temporarily enabled import role before verification:
...
`mesh-backup` creates one authenticated archive containing exact coordinated control and identity state plus canonical master/admin credentials. It requires a stopped server, takes both store locks in server order, proves the bound control-v2 through control-v14 master/admin credentials, validates every active or staged CA, host certificate, issuance, signing key, and sealed identity value, and creates the archive without overwriting any path. The current server writes control v14, where topology, per-network DNS and optional Linux native-resolver settings, relay settings, node issuing-CA identity, crash-durable CA rotation and firewall canaries, explicit firewall pause state, routed-subnet transfer and route-profile receipts, weighted route policies, node/group-scoped firewall selectors, and the security-group catalog are represented. Existing v2 nodes migrate to `unassigned`, existing v3 networks migrate to disabled DNS on canonical port 53, existing v4 networks migrate to disabled relays with an exact empty selection, existing v5 certificates are bound to their current CA, existing v6 networks migrate to stable firewall rollout state, existing v7 stable or in-flight canaries gain an unpaused zero value, existing v8 networks gain an empty route-transfer receipt, existing v9 networks gain an empty route-profile edit receipt, existing v10 networks gain empty route-policy state, existing v11 networks gain disabled native resolver state with an empty search domain, existing v12 networks gain no scoped firewall fields, and existing v13 networks gain the canonical security-group catalog without changing effective policy. None of these compatibility migrations change signed configuration bytes, revisions, or timestamps. The 256-bit backup key is generated and held separately. A live version-1 control store must complete one successful upgraded server startup before capture; unbound version-1 archives are deliberately rejected by the verifier and restore path.
...
That same authenticated bound control-v2 through control-v14 archive is the only supported JSON-to-PostgreSQL import source. `mesh-storage` does not scrape a live data directory and never takes a DSN on its command line.
...
`make postgres-mobile-runtime-smoke` uses one exact loopback-only PostgreSQL 17 container to prove that a current control-v14 backup in the compiled import range is accepted, initializes empty reconstructible schema-v8 telemetry, persists a non-empty mobile runtime record through the production adapter, reads it through a second independent application pool, commits an identical concurrent transition exactly once, verifies continuous receipts, and removes only its exact labeled container. This is a local storage/composition proof, not a persistent-environment rollout or physical iOS tunnel result.
...
roles, and removes all private workspaces. The smoke stops at unsigned bundle
v1. Protected native signing plus `mesh-package build-darwin-signed` produces
final bundle v2; run the separate [Darwin final signed-bundle security
gate](docs/darwin-package-security.md) against that result. Production
authoring requires both its package receipt and the matching fresh native
code-signing receipt. These paths do not install or activate launchd, apply
extended ACLs, perform notarization, or make a native macOS lifecycle claim.
...
A current Apple Silicon partial run passes every enabled native path,
installer-gate, release-layout, exact-child, process-group, and reap test and
publishes a source-bound v3 receipt. It explicitly records no bundle and no
system launchctl mutation, so the full verifier rejects it as incomplete. The
native adapters account for macOS 26.5 rejecting redundant
`O_NOFOLLOW | O_NOFOLLOW_ANY` opens and applying umask to symlink mode bits;
these compatibility results do not make the unsigned or policy-free Node
staging bundle releasable.
