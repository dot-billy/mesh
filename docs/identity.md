...
The iOS host keeps the resulting Mesh session and one-time enrollment token only in an ephemeral in-memory URL session and request envelope. It asks Apple to save the non-secret VPN origin before creating the server enrollment, then passes the returned token directly to the Packet Tunnel extension through `startTunnel(options:)`. Apple's VPN permission prompt remains mandatory and cannot be silently approved by Mesh.
