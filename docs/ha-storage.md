...
- Extend the passing clean-promotion and deterministic receipt-resolution drills to sustained mixed control and identity writes. Cover real transport or controller failures before, during, and after commit, repeated failovers with automated election and fencing, and explicit recovery budgets while retaining monotonic revisions and replica convergence on the authoritative timeline.
- Extend the passing bounded base-backup and archived-WAL drill to points around concurrent mutations and failover. Operate independently protected monotonic recovery authorization, archive retention, timeline history, backup and WAL integrity monitoring, and measured RPO/RTO. Then repeat full session, enrollment, signed-state, and revocation validation under those production conditions.
- Exercise intended node, session, and heartbeat rates and both document bounds. Measure row-lock waits, latency, WAL and receipt growth, vacuum pressure, recovery time, and memory against explicit budgets.
- Repeat the passing isolated role and TLS drill against the target database and every fallback route. Install and rotate the private CA through that platform's OS trust-management path, source role secrets from the deployed secret manager, exercise the actual certificate and password lifecycle and crash collector, and confirm that no DSN or credential enters arguments, logs, diagnostics, or ambient `PG*` state.
- Enforce trusted-edge distributed login and OIDC limits. PostgreSQL storage does not make the current in-process pre-authentication budgets distributed, and Mesh does not trust forwarded client-IP headers.
- Alert on database authority or readiness loss, receipt uncertainty, pool saturation, lock wait, revision drift, WAL and backup health, and recovery objectives. Inject disconnects before, during, and after commit and receipt resolution.
