...
encryption with no France distribution, and now reports `Waiting for Review`.
The saved external-beta record contains the bounded test/review information
and individual tester `wtmuller.medi@gmail.com` with automatic notification.
The tester currently reports `No Builds Available`; Apple must approve Beta
App Review before the invitation exposes build 1. This live App Store Connect
state is operational evidence, not a cryptographic receipt or authorization
for public App Store/Custom App distribution.
