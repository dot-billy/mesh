...
checkout is based on commit
`349d26e33e3fe61a25bd37930ca335d751b302a6`.
...
| Mesh Tunnel for iPhone and iPad | A separate UIKit containing app and Packet Tunnel extension compile as one unsigned universal simulator product with exact development/TestFlight/Custom App entitlement maps. Framework v5 keeps enrollment and every node secret inside the extension, verifies bootstrap and signing-key continuity, performs same-key certificate renewal, rotates the agent credential through a crash-recoverable pending device-only Keychain item, publishes strict config/certificate/engine-bound mobile runtime evidence while scheduled, quarantines ambiguous authorization failure, and exposes a deletion-only exact-node local identity-removal path. Configuration slots are erased only after the current credential, pending credential, and private key are deleted; the non-authority handoff HMAC and high-water floors remain for authentication and rollback protection. Desired-state mismatch stops the active tunnel and converges through the next full start; active in-place hot reload is not claimed. The current host source rediscovers exactly one Mesh provider, can start an existing authenticated local identity without re-enrollment, requests stop, and inspects a request-ID-bound extension outcome. Running status is produced from the real coordinator with configuration revision, certificate generation, engine identity, and directional Apple callback counters; the UI explicitly does not label those counters or `NEVPNStatus` as peer or end-to-end connectivity. The onboarding surface is scrollable for compact iPhones and enlarged text. The cross-checkout reproducible v5 framework tree and clean-source unsigned simulator app statically expose runtime-report and identity-removal symbols, emit only eighteen fixed reviewed log codes, and pass the ten-file Tunnel security gate with the exact 29-module dependency inventory, two empty privacy manifests, no High/Critical or published-fix vulnerability finding, and empty secret scans. Native-host Go tests still prove authenticated direct loopback UDP packets, certificate identity, empty relay state, post-rebind traffic, and repeatable shutdown. Signed version `0.1.0` build `1` remains the earlier framework-v4 pre-start-lifecycle artifact and does not contain the current host controls or framework-v5 changes. A read-only App Store Connect check on 2026-07-25 showed that build in `Testing`, expiring in 90 days, and installed by one invited tester that day on a physical iPhone; it showed no sessions, crashes, or feedback. | The v5 receipts and current host controls are local source evidence, not independent clean-host, signed, uploaded, installed, or physical-device evidence. The existing v4 upload and installation are controlled TestFlight qualification, not release authority, public App Store distribution, or a support claim. Its launch and VPN permission screens do not prove authenticated enrollment, tunnel runtime, or packets. Enrollment, settings, Network Extension callbacks, physical Keychain custody/deletion, UDP, packet, roaming, suspension, resource use, log behavior, heartbeat cadence, renewal, credential rotation, desired-state convergence, revocation cutoff, response-loss recovery, reinstall, and transfer remain unproved on a physical iPhone or iPad. License approval, final App Store privacy/export review, corrected build `2` signing, upload, review, and installed TestFlight execution, Custom App distribution, independent CI reproduction, and the physical lifecycle matrix remain required. |
...
This 2026-07-26 update supersedes the table row's earlier pending-build-2
wording. The strictly verified framework-v5 successor was uploaded as
`0.1.0 (2)`. App Store Connect reports the binary `VALID`, accepted
`usesNonExemptEncryption=false`, and made it ready for internal testing. The
build and tester are attached to the `Mesh Tunnel External Testers` group, and
the Beta App Review submission is `APPROVED`; the external state is
`IN_BETA_TESTING`. Build `2` is not yet installed or physical-device evidence.
...
records static engine linkage and source-wired Apple packet flow but no
...
- only the shared App Group and handoff Keychain group in the containing app,
  and only the Packet Tunnel capability, shared App Group, handoff group, and
  extension-only identity Keychain group in the extension;
- a containing app that prepares at most one exact provider configuration with
  only a canonical non-secret HTTPS origin, then sends one canonical enrollment
  request directly to the extension as the sole start option. It clears the
  one-use token before the call returns and never stores it in VPN preferences
  or the App Group;
- exact-provider rediscovery plus controls to start an existing authenticated
  ...
  engine identity, and directional Apple callback counters. The UI does not
  present those counters or `NEVPNStatus` as peer or end-to-end connectivity,
...
- a shared handoff authentication key stored as non-synchronizing,
  ...
  available only to the extension identity access group;
- an extension-owned enrollment session that performs the token-scoped
  ...
  the extension-only Keychain group, sends only the public key and agent-bearer
...
- an extension-owned lifecycle session that opens only existing credentials,
...
- an extension provider wired through that coordinator and the statically
  linked Go enrollment, lifecycle, and engine sessions. Pre-start refresh and
  any ready activation complete before engine construction. Its two
  source-wired `packetFlow` tasks start only after engine preparation,
  settings, pump, and engine startup; subsequent `NWPathMonitor` updates invoke
  bounded UDP rebind, and any packet or rebind failure cancels the tasks and
  fails the tunnel closed;
- an extension-only Unified Logging wrapper that accepts a closed enum of
  eighteen fixed start, stop, configuration-rejection, enrollment-rejection,
  enrollment-failure, lifecycle-deferred, lifecycle-failure,
  agent-authorization-rejection, unavailable-engine, network-rebind-failure,
  packet-flow-failure, status-request, and identity-removal codes; no stop
...
- an unsigned simulator build receipt proving one universal iPhone/iPad host,
  ...
  project source digests. It records a statically linked engine and
  source-wired Apple packet callbacks, but no runtime packet connection or
  physical-device validation. It also binds the current request-bound status,
...
- a separate Go packet-session framework source boundary pinned to Nebula 1.10.3 and
  ...
  X25519 private key in the extension-only Data Protection Keychain, return
...
This remains source/link and reproducibility evidence, not a tunnel. Apple's
documented `NEPacketTunnelFlow` interface presents packet read/write callbacks.
Although the current upstream mobile integration discovers a utun descriptor,
Mesh has not adopted that technique: pinned Nebula 1.10.3 also exposes an
in-memory `overlay.UserDevice`. A bounded Mesh adapter validates and copies
exact IPv4/IPv6 packets between that device's pipes and the exported engine
callback loop, bounds packets to 65,575 bytes, rejects malformed lengths, and
proves both directions and close behavior in Go tests. A separate
native-host test constructs two real pinned Nebula controls with ephemeral
certificates, callback devices, and actual loopback UDP sockets; an ICMP
request and reply cross the authenticated direct tunnel unchanged, certificate
fingerprints match, relay state stays empty, and 20 repeated start/packet/stop
runs pass after the adapter translates close into Nebula's expected
`os.ErrClosed` contract. Each run also rebinds the member's real UDP listener
through Nebula's mobile rebind method and proves another authenticated packet
arrives. The engine session exports that callback boundary to Swift and is
statically linked into the unsigned simulator extension. The coordinator
proves bounded queue backpressure, startup ordering, reverse cleanup,
network-path rebind, lifecycle gating, scheduled runtime evidence, and
source-wired Apple packet-flow handling. The current signed archive and
TestFlight upload contain the earlier framework-v4 pre-start symbols; they do
not contain the new v5 renewal, rotation, runtime-report, or identity-removal
surface. No valid device handoff has started either extension and no live
server has consumed a token through the extension-owned ceremony. The
persistent remote test control plane still predates the new mobile-runtime
route: a fresh authenticated read-only networks/nodes/projection probe on
2026-07-25 returned HTTP 200, HTTP 200, and HTTP 404 respectively. The
framework maps an absent `POST` route reported as HTTP 404 or 405 to the exact
`unsupported` outcome; the provider permits startup and stops scheduled
reporting rather than misclassifying the device as accepted, stale, or
revoked. This compatibility path can keep an older server from blocking a
basic Tunnel start, but it provides no server-visible mobile evidence and is
not physical connectivity proof. The persistent service was not changed
...
executed callback cancellation, accepted Apple network settings, iOS UDP
operation, resource measurement, and real-device packet evidence remain
