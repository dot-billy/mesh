...
| Mesh Tunnel for iPhone and iPad | The UIKit host, Packet Tunnel extension, framework v5, shared device-only Keychain custody, lifecycle refresh, runtime evidence, and identity removal compile and pass local source/security gates. Build 8 follows Mobile Nebula's provision-first/connect-second sequence: validate one manager, self-enroll, perform bounded Go enrollment and authenticated site activation in the host, enable/save/reload the manager, refresh with the existing credential, write one exact Keychain-backed start authorization, and then start the provider. The provider rejects Settings-only starts and performs only local configuration, Apple settings, and Nebula startup before completing Apple's callback; reporting begins afterward. Exact intent and candidate checks support interrupted enrollment without another token. The successor source contains unexpected Go enrollment/recovery panics at the mobile boundary with one fixed non-secret error and presents complete retained authority as `Recover enrolled VPN`, without repeating OIDC or requesting a token. Builds 2 through 8 are externally distributed. Build 5 physically proved OIDC, network selection, and host self-enrollment; Build 6 returned `apple-vpn-disconnected` with zero provider requests; Build 7 stalled before token issuance. Build 8 physically completed OIDC and server enrollment: audit proves the existing iPhone enrollment was reissued and node `E3kVivz4BJPeBgvh` became active at `2026-07-27T20:39:01Z`. The containing app then unexpectedly terminated during post-enrollment host handoff. | Build 8 proves the server transaction but not the exact local termination substage; App Store Connect and Xcode had no crash log when checked. No build has proved committed local configuration, a running extension, recovery on a physical device, or a packet callback. Panic containment and no-login recovery are source-tested only. Installed-device lifecycle, credentials, recovery, roaming, response-loss, crash/restart, packets, and successor distribution remain required. |
...
terminated in post-enrollment host handoff. That result proves neither local
site activation nor a running tunnel or packet exchange.
...
through 8 reached controlled external testing. Build 5 physically completed
...
app then unexpectedly terminated during post-enrollment host handoff. No
physical run has yet proved committed local configuration, started the
framework-v5 runtime, recovered the interrupted enrollment, or exchanged a
packet. The successor contains unexpected Go enrollment/recovery panics and
offers complete retained authority as `Recover enrolled VPN` without another
login or token. That behavior remains source-tested only.
