// Package windowspackagesecurity parses the canonical local security evidence
// produced for one exact final signed Windows node-bundle candidate.
package windowspackagesecurity
...
var expectedRuntime = RuntimeIdentity{
	...
	Version:                "v1.10.3",
	WindowsBuildLockSHA256: "abf5790a7c94847dd5d5bb0898fb2b72081e121e32b90e05d333309d33ea212d",
}

var expectedUpstream = map[string]UpstreamIdentity{
	"amd64": {
		ArchiveSHA256: "7e8f26eef9394c842d4eb73610e3e63b965a93b803e366eb42c9b548bd1a70b8",
		ArchiveSize:   16342405, AssetID: 351844826, AssetName: "nebula-windows-amd64.zip",
		LockSHA256: "9f0515702d74a0911263a160754948220e41b5da7048f48adf67355b3816dd7c", Version: "v1.10.3",
	},
	"arm64": {
		ArchiveSHA256: "c8e3c50c54708fed2e519dce7d455b639e5a9c2d124933b1a74aa3c2ba9632ae",
		ArchiveSize:   14936616, AssetID: 351844824, AssetName: "nebula-windows-arm64.zip",
		LockSHA256: "9f0515702d74a0911263a160754948220e41b5da7048f48adf67355b3816dd7c", Version: "v1.10.3",
	},
}
...
func ParseReceipt(raw []byte) (Receipt, error) {
	if len(raw) < 1 || len(raw) > MaxReceiptSize {
		return Receipt{}, fmt.Errorf("Windows package security receipt size must be between 1 and %d bytes", MaxReceiptSize)
	}
	if err := releasetrust.ValidateStrictJSON(raw); err != nil {
		return Receipt{}, fmt.Errorf("invalid Windows package security receipt JSON: %w", err)
	}
	decoder := json.NewDecoder(bytes.NewReader(raw))
	decoder.DisallowUnknownFields()
	var receipt Receipt
	if err := decoder.Decode(&receipt); err != nil {
		return Receipt{}, fmt.Errorf("decode Windows package security receipt: %w", err)
	}
	var extra any
	if err := decoder.Decode(&extra); !errors.Is(err, io.EOF) {
		return Receipt{}, errors.New("Windows package security receipt contains trailing content")
	}
	...
	if err != nil {
		return Receipt{}, fmt.Errorf("encode Windows package security receipt: %w", err)
	}
	canonical = append(canonical, '\n')
	if !bytes.Equal(raw, canonical) {
		return Receipt{}, errors.New("Windows package security receipt must be canonical sorted compact JSON followed by one LF")
	}
	return receipt, nil
}

// MatchArtifact proves the receipt covers the exact Windows artifact identity
...
func (receipt Receipt) MatchArtifact(now time.Time, arch, version string, securityFloor uint64, size int64, sha256 string) error {
	...
	if now.IsZero() || verifiedAt.After(now.Add(maxFutureSkew)) {
		return errors.New("Windows package security receipt verification time is in the future")
	}
	if now.Sub(verifiedAt) > maxReceiptAge {
		return errors.New("Windows package security receipt is older than 24 hours")
	}
	if receipt.Candidate.Architecture != arch || receipt.Candidate.Version != version ||
		...
		return errors.New("Windows package security receipt differs from the release-selected artifact identity")
	}
	return nil
}

func validateReceipt(receipt Receipt) error {
	if receipt.Schema != Schema || receipt.Candidate.Schema != "mesh-windows-node-bundle-v3" {
		return errors.New("unsupported Windows package security receipt schema")
	}
	arch := receipt.Candidate.Architecture
	upstream, ok := expectedUpstream[arch]
	if !ok {
		return errors.New("Windows package security receipt architecture is invalid")
	}
	if receipt.Candidate.Version == "" || len(receipt.Candidate.Version) > 128 || strings.TrimSpace(receipt.Candidate.Version) != receipt.Candidate.Version ||
		...
		receipt.Candidate.GoVersion != "go1.26.5" || receipt.Candidate.FileCount != 8 || receipt.Candidate.DirectoryCount != 9 ||
		receipt.Candidate.TotalBytes < 1 || receipt.Candidate.TotalBytes > 256<<20 || receipt.Candidate.Runtime != expectedRuntime ||
		receipt.Candidate.Upstream != upstream || !validDigest(receipt.Candidate.PackageJSONSHA256) {
		return errors.New("Windows package security candidate identity, provenance, or bounds are invalid")
	}
	...
	expectedFiles := []string{
		...
		"bin/dist/windows/wintun/bin/" + arch + "/wintun.dll", "bin/meshctl.exe",
		"bin/nebula-cert.exe", "bin/nebula.exe", "package.json", "share/licenses/nebula/LICENSE",
	}
	if len(receipt.Candidate.Files) != len(expectedFiles) {
		return errors.New("Windows package security receipt file set is incomplete")
	}
	var total int64
	for _, name := range expectedFiles {
		record, found := receipt.Candidate.Files[name]
		if !found {
			return fmt.Errorf("Windows package security receipt is missing %s", name)
		}
	...
	if total != receipt.Candidate.TotalBytes || receipt.Candidate.Files["package.json"].SHA256 != receipt.Candidate.PackageJSONSHA256 {
		return errors.New("Windows package security receipt file totals or package identity are inconsistent")
	}
	...
	if receipt.SBOM.SyftVersion != "1.44.0" || receipt.SBOM.SyftSchema != "16.1.3" || receipt.SBOM.SyftPackageCount != 59 ||
		...
		return errors.New("Windows package security SBOM versions or exact package counts are invalid")
	}
	if receipt.ScannerBoundary.ArtifactAndScan != "stable candidate, networkless read-only non-root scanners, no Docker socket" ||
		...
		return errors.New("Windows package scanner isolation boundary is invalid")
	}
	if receipt.SecretScan.GitleaksVersion != "v8.30.1" ||
		receipt.SecretScan.Policy != "default rules over exact package metadata, Wintun notices, license, and all four PEs' strings; only the exact public oauth2 v0.36.0 Go checksum is allowlisted" ||
		...
		return errors.New("Windows package secret-scan policy or empty reports are invalid")
	}
	if receipt.VulnerabilityScan.GrypeVersion != "0.112.0" ||
		...
		return errors.New("Windows package vulnerability-scan policy, database, or exact admitted results are invalid")
	}
	if !canonicalUTC(receipt.VerifiedAt) {
		return errors.New("Windows package security verification time must be canonical UTC RFC3339")
	}
	...
	if databaseBuilt.After(verifiedAt) || verifiedAt.Sub(databaseBuilt) > maxDatabaseAge {
		return errors.New("Windows package vulnerability database was not current at verification time")
	}
	return nil
}
...
func validRecord(record DigestRecord, minimum, maximum int64, label string) error {
	if !validDigest(record.SHA256) || record.Size < minimum || record.Size > maximum {
		return fmt.Errorf("Windows package security %s digest or size is invalid", label)
	}
	return nil
}
