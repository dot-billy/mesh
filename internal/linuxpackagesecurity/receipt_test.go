...
func TestReceiptRejectsStaleAndFutureReleaseUse(t *testing.T) {
	parsed, err := ParseReceipt(encodeFixture(t, fixtureReceipt()))
	if err != nil {
		t.Fatal(err)
	}
...
func TestReceiptRejectsCanonicalAndPolicyDrift(t *testing.T) {
	valid := encodeFixture(t, fixtureReceipt())
...
func fixtureReceipt() Receipt {
	...
	for _, path := range paths {
		receipt.Candidate.Files[path] = record("1", 1)
	}
