...
// Build creates one deterministic Darwin node staging bundle at a new output
// path. The bundle is not an installer and applies no macOS host policy.
func Build(options BuildOptions) (BuildResult, error) {
	...
	if _, err := nebulaobserverartifact.VerifyDarwinStagedDirectory(options.NebulaRuntimeDirectory, arch); err != nil {
		return BuildResult{}, fmt.Errorf("verify exact security-patched Nebula runtime tree: %w", err)
	}
	// The complete runtime tree verification above is intentionally not
	// described as a transactional tree anchor: selected files are reopened
	// below. Every copied byte is rechecked against its exact locked size and
	...
	for bundlePath, sourcePath := range policy.inputs {
		...
		default:
			return BuildResult{}, fmt.Errorf("input policy for %s is unsupported", bundlePath)
		}
...
func requireBuildHost(goos string) error {
	if goos != "linux" {
		return errors.New("Darwin staging-bundle construction requires a Linux packaging host for exact POSIX staged-tree and publication-mode verification")
	}
	return nil
}
...
func buildWithPolicy(options BuildOptions, policy bundlePolicy, contents map[string][]byte) (BuildResult, error) {
	...
	goVersion, err := verifyMeshBinary(contents["bin/meshctl"], "mesh/cmd/meshctl", policy.arch, expectedIdentity)
	if err != nil {
	...
	metadata := Package{
		...
		GoVersion:              goVersion, Target: Target{OS: "darwin", Arch: policy.arch},
	...
	temporaryName, err := randomName(".mesh-darwin-bundle-")
	if err != nil {
		return BuildResult{}, err
	}
