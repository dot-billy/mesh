...
func parseHeaderBlock(block []byte) (*tar.Header, error) {
	if len(block) != int(tarBlockSize) {
		return nil, errors.New("tar header block must be exactly 512 bytes")
	}
	reader := tar.NewReader(bytes.NewReader(block))
	header, err := reader.Next()
	if err != nil {
		return nil, fmt.Errorf("parse USTAR header: %w", err)
	}
	if header.Format != tar.FormatUSTAR {
		return nil, fmt.Errorf("tar member uses %s, want USTAR", header.Format)
	}
	return header, nil
}
...
func readExact(reader io.Reader, destination []byte, label string) error {
