...
RUNTIME = {
    ...
    "darwin_build_lock_sha256": "007f2ede3ef625ee48de3b5c616c43b3d5698903c09649f2b6a1cbed8d9392b7",
...
def payload_modes(_arch: str) -> dict[str, int]:
    return {
        "Library/LaunchDaemons/io.mesh.node-agent.plist": 0o444,
        "bin/meshctl": 0o555,
        "bin/nebula-cert": 0o555,
        "bin/nebula": 0o555,
        "share/doc/mesh/launchd/README.md": 0o444,
        "share/licenses/nebula/LICENSE": 0o444,
    }


def directory_paths(_arch: str) -> set[str]:
    return {
        "Library", "Library/LaunchDaemons", "bin", "share", "share/doc", "share/doc/mesh",
        "share/doc/mesh/launchd", "share/licenses", "share/licenses/nebula",
    }
...
def validate_inspection(document: dict[str, Any], staged: pathlib.Path, artifact: pathlib.Path) -> dict[str, Any]:
    ...
    require(document["schema"] == "mesh-darwin-security-candidate-inspection-v1", "candidate inspection schema is invalid")
    ...
    require(isinstance(target, dict) and set(target) == {"os", "arch"} and target.get("os") == "darwin" and target.get("arch") in {"amd64", "arm64"}, "candidate target is invalid")
    arch = target["arch"]
    require(package.get("runtime") == RUNTIME, "candidate Darwin runtime provenance differs from the exact security-patched lock")
    ...
    for name, entry in observed_entries.items():
        ...
        files[name] = record
        total += record["size"]
    require(document["file_count"] == 7 and document["directory_count"] == 9 and document["total_bytes"] == total, "candidate inspection counts are inconsistent")
    return {"artifact": artifact_record, "files": dict(sorted(files.items())), "package": package}


def expected_artifacts(_arch: str) -> set[tuple[str, str, str, str]]:
    return {(name, version, "go-module", location) for (name, version), locations in PACKAGE_LOCATIONS.items() for location in locations}


def validate_syft(document: dict[str, Any], arch: str) -> tuple[set[str], int]:
    descriptor = document.get("descriptor")
    require(isinstance(descriptor, dict) and descriptor.get("name") == "syft" and descriptor.get("version") == "1.44.0", "Darwin package SBOM descriptor is invalid")
    schema = document.get("schema")
    require(isinstance(schema, dict) and schema.get("version") == "16.1.3", "Darwin package Syft schema is unexpected")
    source = document.get("source")
    require(isinstance(source, dict) and source.get("type") == "directory", "Darwin package SBOM source is not the staged directory")
    artifacts = document.get("artifacts")
    expected = expected_artifacts(arch)
    require(isinstance(artifacts, list) and len(artifacts) == len(expected) == 52, "Darwin package SBOM must contain exactly 52 admitted package locations")
    observed: set[tuple[str, str, str, str]] = set()
    purls: set[str] = set()
    for artifact in artifacts:
        require(isinstance(artifact, dict), "Darwin package SBOM artifact is invalid")
        name, version, kind = artifact.get("name"), artifact.get("version"), artifact.get("type")
        locations = artifact_locations(artifact)
        require(isinstance(name, str) and isinstance(version, str) and isinstance(kind, str) and len(locations) == 1, "Darwin package SBOM identity or location is invalid")
        location = next(iter(locations))
        observed.add((name, version, kind, location))
        purl = artifact.get("purl")
        require(kind == "go-module" and isinstance(purl, str) and purl.startswith("pkg:golang/"), f"Darwin package SBOM lacks an exact Go purl: {name}")
        purls.add(purl)
    require(observed == expected, "Darwin package SBOM inventory differs from the exact allowlist")
    return purls, len(artifacts)


def finalize(args: argparse.Namespace) -> None:
    work = pathlib.Path(args.work_dir)
    require(work.is_dir() and not work.is_symlink(), "Darwin package verification workspace is unsafe")
    ...
    require(all(isinstance(item, dict) for item in (syft, spdx, grype, database)), "Darwin package scanner output is not an object")
    purls, syft_count = validate_syft(syft, arch)
    spdx_count = validate_spdx(spdx, purls)
    require(spdx_count == 53, "Darwin package SPDX inventory must contain exactly 53 packages")
    ...
    require(receipt_path.parent == work, "Darwin package receipt must be written inside the verification workspace")
...
def main() -> int:
    try:
        ...
        print(f"Darwin package security verification: {exc}", file=sys.stderr)
        return 1
    return 0
