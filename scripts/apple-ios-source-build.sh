#!/bin/bash
# Build and inspect the unsigned iPhone/iPad Admin simulator application.
set -euo pipefail
umask 077

if [[ "$#" -ne 1 ]]; then
  printf 'usage: apple-ios-source-build.sh <new-absolute-derived-data>\n' >&2
  exit 2
fi
if [[ "$(uname -s)" != Darwin ]]; then
  printf 'Apple iOS source build requires a native Mac\n' >&2
  exit 77
fi

derived_data="$1"
if [[ "${derived_data}" != /* || -e "${derived_data}" ]]; then
  printf 'Apple iOS source build output must be a new absolute path\n' >&2
  exit 2
fi
...
for name in \
  ...
  if [[ -n "${!name:-}" ]]; then
    printf 'unsigned Apple iOS source build received release credential variable %s\n' "${name}" >&2
    exit 1
  fi
done
...
if [[ ! -d "${parent}" || -L "${parent}" ]]; then
  printf 'Apple iOS source build output parent must be one existing physical directory\n' >&2
  exit 2
fi

(
  cd "${desktop_root}"
  "${MESH_FLUTTER}" build ios --simulator --debug --config-only \
...
python3 "${repo_root}/scripts/apple_source_artifact_receipt.py" \
  ...
  --output "${derived_data}/mesh-apple-ios-simulator-source-receipt.json"
