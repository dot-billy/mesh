#!/bin/bash
# Produce create-only security evidence for one exact unsigned iOS Admin simulator app.
set -Eeuo pipefail
umask 077

say() { printf '%s\n' "$*"; }
die() { printf 'Apple iOS Admin security baseline: %s\n' "$*" >&2; exit 1; }

[[ $# -eq 3 ]] || die "usage: $0 /absolute/Runner.app /absolute/source-receipt.json /absolute/flutter"
[[ "$(uname -s)" == Darwin ]] || die "this gate requires a native Mac"

script_dir="$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd -P)"
repo_root="$(cd -- "${script_dir}/.." && pwd -P)"
desktop_root="${repo_root}/desktop"
verify_script="${script_dir}/apple_ios_admin_security_verify.py"
gitleaks_config="${repo_root}/.gitleaks-apple-ios-admin.toml"
app="$1"
source_receipt="$2"
flutter="$3"

[[ "${app}" == /* && -d "${app}" && ! -L "${app}" && "${app##*/}" == "Runner.app" ]] || die "app must be an absolute unlinked Runner.app directory"
...
if actual.get("frameworkVersion") != declared["version"]:
    raise SystemExit(
        "Apple iOS Admin security baseline: Flutter version differs from the pinned declaration"
    )
if actual.get("frameworkRevision") != declared["framework_commit"]:
    raise SystemExit(
        "Apple iOS Admin security baseline: Flutter commit differs from the pinned declaration"
    )
PY
...
work_dir="$(mktemp -d "${temporary_parent%/}/mesh-apple-ios-admin-security.XXXXXX")"
[[ -d "${work_dir}" && ! -L "${work_dir}" && "${work_dir}" == "${temporary_parent%/}/mesh-apple-ios-admin-security."* ]] || die "mktemp returned an unsafe workspace"
chmod 0700 "${work_dir}"
install -d -m 0700 \
  "${work_dir}/scan-root" \
...
cleanup() {
  ...
  if [[ "${publish_complete}" != true && -n "${published_dir}" && -d "${published_dir}" && ! -L "${published_dir}" ]]; then
    case "${published_dir}" in
      "${repo_root}/bin/apple-ios-admin-security/"*) rm -rf -- "${published_dir}" ;;
      *) printf 'Refusing to remove unexpected partial publication %s\n' "${published_dir}" >&2 ;;
    esac
  fi
  if [[ -n "${work_dir:-}" && -d "${work_dir}" && "${work_dir}" == "${temporary_parent%/}/mesh-apple-ios-admin-security."* ]]; then
...
say "Taking a stable snapshot of the exact unsigned iOS Admin simulator app"
/usr/bin/ditto --norsrc --noextattr "${app}" "${work_dir}/scan-root/Runner.app"
...
if observed != (
    ...
    raise SystemExit(
        "Apple iOS Admin security baseline: stable app snapshot differs from its source receipt"
    )
PY
...
say "Scanning the exact iOS Admin SBOM offline with Grype ${grype_version}"
set +e
docker "${scanner_base[@]}" \
...
say "Preparing metadata and secret-scan text from every regular iOS app file"
...
python3 - "${work_dir}/scan-root/Runner.app" "${work_dir}/app-strings" <<'PY'
import json
import os
import pathlib
import plistlib
...
PY
python3 - "${work_dir}/scan-root/Runner.app/Frameworks/App.framework/flutter_assets/NOTICES.Z" "${work_dir}/metadata/NOTICES.txt" <<'PY'
import gzip
import os
import pathlib
import sys
...
say "Scanning bound metadata and iOS app strings with Gitleaks ${gitleaks_version}"
for scan in metadata app-strings; do
  ...
  docker "${scanner_base[@]}" \
    ...
    -v "${gitleaks_config}:/config/apple-ios-admin.toml:ro" "${gitleaks_image}" dir /scan \
    --config=/config/apple-ios-admin.toml --no-banner --no-color --redact=100 \
    --max-target-megabytes=128 --max-archive-depth=0 --max-decode-depth=3 --timeout=120 \
    --report-format=json --report-path="/output/${report}"
done

say "Binding iOS app, dependency, license, privacy, vulnerability, and secret evidence"
...
output_root="${repo_root}/bin/apple-ios-admin-security"
...
say "PASS: exact unsigned iOS Admin simulator security evidence verified"
