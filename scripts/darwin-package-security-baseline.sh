...
say() { printf '%s\n' "$*"; }
die() { printf 'Darwin package security baseline: %s\n' "$*" >&2; exit 1; }

[[ $# -eq 1 && -n "$1" ]] || die "usage: $0 /clean/absolute/mesh-darwin-bundle.tar"

script_dir="$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd -P)"
repo_root="$(cd -- "${script_dir}/.." && pwd -P)"
verify_script="${script_dir}/darwin_package_security_verify.py"
...
work_dir="$(mktemp -d "${temporary_parent%/}/mesh-darwin-package-security.XXXXXX")"
[[ -d "${work_dir}" && ! -L "${work_dir}" && "${work_dir}" == "${temporary_parent%/}/mesh-darwin-package-security."* ]] || die "mktemp returned an unsafe workspace"
chmod 0700 "${work_dir}"
...
cleanup() {
  ...
  if [[ "${publish_complete}" != true && -n "${published_dir}" && -d "${published_dir}" && ! -L "${published_dir}" ]]; then
    case "${published_dir}" in
      "${repo_root}/bin/darwin-package-security/"*) rm -rf -- "${published_dir}" ;;
      *) printf 'Refusing to remove unexpected partial publication %s\n' "${published_dir}" >&2 ;;
    esac
  fi
  if [[ -n "${work_dir:-}" && -d "${work_dir}" && "${work_dir}" == "${temporary_parent%/}/mesh-darwin-package-security."* ]]; then
...
say "Building the current exact Darwin package verifier"
cd -- "${repo_root}"
...
say "Stably validating and staging the exact Darwin candidate through production policy"
"${work_dir}/build/mesh-package" inspect-darwin \
  --artifact "${bundle}" \
...
say "Generating exact Darwin Syft and SPDX inventories offline"
...
say "Scanning the exact Darwin package SBOM offline with Grype ${grype_version}"
set +e
...
say "Scanning exact Darwin package text and all three Mach-O executables' strings with Gitleaks ${gitleaks_version}"
for path in package.json Library/LaunchDaemons/io.mesh.node-agent.plist share/doc/mesh/launchd/README.md share/licenses/nebula/LICENSE; do
  destination="${work_dir}/text/${path//\//__}"
  install -m 0400 "${work_dir}/staged/${path}" "${destination}"
done
for relative in bin/meshctl bin/nebula bin/nebula-cert; do
  name="${relative//\//__}"
...
say "Binding Darwin candidate, exact inventory, vulnerability, and secret evidence"
...
output_root="${repo_root}/bin/darwin-package-security"
