...
say() { printf '%s\n' "$*"; }
die() { printf 'Linux package security baseline: %s\n' "$*" >&2; exit 1; }

[[ $# -eq 1 && -n "$1" ]] || die "usage: $0 /clean/absolute/mesh-linux-bundle.tar"

script_dir="$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd -P)"
repo_root="$(cd -- "${script_dir}/.." && pwd -P)"
verify_script="${script_dir}/linux_package_security_verify.py"
...
work_dir="$(mktemp -d "${temporary_parent%/}/mesh-linux-package-security.XXXXXX")"
[[ -d "${work_dir}" && ! -L "${work_dir}" && "${work_dir}" == "${temporary_parent%/}/mesh-linux-package-security."* ]] || die "mktemp returned an unsafe workspace"
chmod 0700 "${work_dir}"
...
cleanup() {
  ...
  if [[ "${publish_complete}" != true && -n "${published_dir}" && -d "${published_dir}" && ! -L "${published_dir}" ]]; then
    case "${published_dir}" in
      "${repo_root}/bin/linux-package-security/"*) rm -rf -- "${published_dir}" ;;
      *) printf 'Refusing to remove unexpected partial publication %s\n' "${published_dir}" >&2 ;;
    esac
  fi
  if [[ -n "${work_dir:-}" && -d "${work_dir}" && "${work_dir}" == "${temporary_parent%/}/mesh-linux-package-security."* ]]; then
...
say "Building the current exact Linux package verifier"
cd -- "${repo_root}"
...
say "Stably validating and staging the exact candidate through production policy"
"${work_dir}/build/mesh-package" inspect-linux \
  --artifact "${bundle}" \
...
say "Scanning the exact package SBOM offline with Grype ${grype_version}"
set +e
docker "${scanner_base[@]}" \
...
say "Scanning exact package text and all four binaries' strings with Gitleaks ${gitleaks_version}"
for path in \
  package.json \
  ...
  share/doc/mesh/systemd/README.md \
  share/licenses/nebula/LICENSE; do
  destination="${work_dir}/text/${path//\//__}"
  install -m 0400 "${work_dir}/staged/${path}" "${destination}"
done
for binary in mesh-install meshctl nebula nebula-cert; do
  strings -a -n 8 "${work_dir}/staged/bin/${binary}" >"${work_dir}/binary-strings/${binary}.strings"
...
say "Binding candidate, exact inventory, vulnerability, and secret evidence"
PYTHONDONTWRITEBYTECODE=1 python3 "${verify_script}" \
  --work-dir "${work_dir}" \
  --verifier "${work_dir}/build/mesh-package" \
  --receipt "${work_dir}/receipt.json"

output_root="${repo_root}/bin/linux-package-security"
...
say "PASS: exact Linux node bundle, production policy, SBOM, current vulnerability policy, and redacted secret scans verified"
