...
say() { printf '%s\n' "$*"; }
die() { printf 'origin image security baseline: %s\n' "$*" >&2; exit 1; }

[[ $# -eq 0 ]] || die "this gate accepts no arguments"

script_dir="$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd -P)"
repo_root="$(cd -- "${script_dir}/.." && pwd -P)"
verify_script="${script_dir}/origin_image_security_verify.py"
gitleaks_config="${repo_root}/.gitleaks-image.toml"
dockerfile="${repo_root}/packaging/origin/Dockerfile"
[[ -f "${verify_script}" && -f "${gitleaks_config}" && -f "${dockerfile}" ]] || die "origin image-security inputs are missing"
...
work_dir="$(mktemp -d "${temporary_parent%/}/mesh-origin-image-security.XXXXXX")"
[[ -d "${work_dir}" && ! -L "${work_dir}" && "${work_dir}" == "${temporary_parent%/}/mesh-origin-image-security."* ]] || die "mktemp returned an unsafe workspace"
chmod 0700 "${work_dir}"
install -d -m 0700 "${work_dir}/buildx" "${work_dir}/db" "${work_dir}/prepared" "${work_dir}/binary-strings"

image_tag="mesh-release-origin:image-security-$$"
published_dir=""
...
cleanup() {
  ...
  if [[ "${publish_complete}" != true && -n "${published_dir}" && -d "${published_dir}" && ! -L "${published_dir}" ]]; then
    case "${published_dir}" in
      "${repo_root}/bin/origin-image-security/"*) rm -rf -- "${published_dir}" ;;
      *) printf 'Refusing to remove unexpected partial publication %s\n' "${published_dir}" >&2 ;;
    esac
  fi
  if [[ -n "${work_dir:-}" && -d "${work_dir}" && "${work_dir}" == "${temporary_parent%/}/mesh-origin-image-security."* ]]; then
    rm -rf -- "${work_dir}"
  fi
  exit "${status}"
}
...
say "Building the pinned linux/amd64 release-origin scratch image"
cd -- "${repo_root}"
...
[[ "${image_id}" =~ ^sha256:[0-9a-f]{64}$ ]] || die "Docker returned an invalid origin image ID"
docker image save --output "${work_dir}/image.tar" "${image_id}"
chmod 0400 "${work_dir}/image.tar"

say "Validating the exact origin filesystem and extracting bounded scan inputs"
PYTHONDONTWRITEBYTECODE=1 python3 "${verify_script}" prepare \
  --archive "${work_dir}/image.tar" \
  --image-id "${image_id}" \
  --output-dir "${work_dir}/prepared"

say "Generating bound Syft and SPDX inventories offline with Syft ${syft_version}"
docker "${scanner_base[@]}" \
...
say "Scanning the bound origin SBOM offline with Grype ${grype_version}"
set +e
docker "${scanner_base[@]}" \
...
say "Scanning exact rootfs text and both binaries with Gitleaks ${gitleaks_version}"
for binary in mesh-origin mesh-healthcheck; do
...
say "Binding origin image, SBOM, database, vulnerability, and secret evidence"
PYTHONDONTWRITEBYTECODE=1 python3 "${verify_script}" finalize \
  --work-dir "${work_dir}" \
  --receipt "${work_dir}/receipt.json"

output_root="${repo_root}/bin/origin-image-security"
if [[ -e "${output_root}" ]]; then
  [[ -d "${output_root}" && ! -L "${output_root}" ]] || die "origin image-security output root is unsafe"
else
...
print(receipt["image"]["docker_image_id"].removeprefix("sha256:"))
print(verified.strftime("%Y%m%dT%H%M%SZ"))
PY
)
image_digest="${publication_identity[0]:-}"
verification_stamp="${publication_identity[1]:-}"
[[ "${image_digest}" =~ ^[0-9a-f]{64}$ && "${verification_stamp}" =~ ^[0-9]{8}T[0-9]{6}Z$ ]] || die "origin receipt returned an invalid publication identity"
...
install -m 0400 "${work_dir}/prepared/image-metadata.json" "${published_dir}/origin-image-metadata.json"
install -m 0400 "${work_dir}/sbom.syft.json" "${published_dir}/mesh-release-origin.syft.json"
install -m 0400 "${work_dir}/sbom.spdx.json" "${published_dir}/mesh-release-origin.spdx.json"
...
say "PASS: exact release-origin image, SBOM, current vulnerability policy, and redacted secret scans verified"
